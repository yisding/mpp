...
#define MODULE_TAG "hal_h264e_vepu511a"
...
typedef struct Vepu511aRoiH264BsCfg_t {
    ...
    RK_U64 qp_adj_en     : 1;
    RK_U64 amv_en        : 1;
} Vepu511aRoiH264BsCfg;

typedef struct VEPU511AH264Fbk_t {
...
typedef struct HalH264eVepu511aCtx_t {
    MppEncCfgSet            *cfg;
    ...
    Vepu511aH264Fbk          feedback;
    Vepu511aH264Fbk          last_frame_fb;
...
static RK_U8 vepu511a_h264_cqm_jvt8p[64] = {
...
static MPP_RET hal_h264e_vepu511a_init(void *hal, MppEncHalCfg *cfg)
{
    HalH264eVepu511aCtx *p = (HalH264eVepu511aCtx *)hal;
    MPP_RET ret = MPP_OK;
    RK_U32 i;
    ...
    if (ret) {
        mpp_loge_f("init vepu buffer failed ret: %d\n", ret);
        goto DONE;
    }
 ...
 */
static void setup_hal_bufs(HalH264eVepu511aCtx *ctx)
{
...
static MPP_RET hal_h264e_vepu511a_prepare(void *hal)
{
...
static RK_U32 update_vepu511a_syntax(HalH264eVepu511aCtx *ctx, MppSyntax *syntax)
{
...
static MPP_RET hal_h264e_vepu511a_get_task(void *hal, HalEncTask *task)
{
    HalH264eVepu511aCtx *ctx = (HalH264eVepu511aCtx *)hal;
...
static void setup_vepu511a_normal(HalVepu511aRegSet *regs)
{
...
static MPP_RET setup_vepu511a_prep(HalVepu511aRegSet *regs, MppEncPrepCfg *prep, HalEncTask *task)
{
    ...
    if (hw_fmt < VEPU5xx_FMT_ARGB1555) {
        ...
        hal_h264e_dbg_flow("input color range %d colorspace %d",
                           prep->range, prep->color);
        ...
        hal_h264e_dbg_flow("use color range %d colorspace %d",
                           cfg_coeffs->dst_range, cfg_coeffs->color);
    } else {
...
static MPP_RET vepu511a_h264e_save_pass1_patch(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx)
{
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
...
static MPP_RET vepu511a_h264e_use_pass1_patch(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx)
{
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
...
static void setup_vepu511a_codec(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx)
{
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
    ...
    reg_frm->synt_sli1.idr_pid        = (slice->slice_type == H264_I_SLICE) ?
                                        slice->idr_pic_id : (RK_U32)(-1);
    ...
    {   /* reorder process */
        ...
        if (MPP_OK == ret) {
            ...
            default : {
                mpp_loge_f("invalid modification_of_pic_nums_idc %d\n",
    ...
    if (slice->slice_type == H264_I_SLICE) {
        ...
        if (!h264e_marking_is_empty(slice->marking)) {
            ...
            do {
                ...
                default : {
                    mpp_loge_f("unsupported mmco 0 %d\n", type);
                    type = 0;
                } break;
                }
...
static void setup_vepu511a_rdo_pred(HalH264eVepu511aCtx *ctx)
{
...
static void setup_vepu511a_rc_base(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx, EncRcTask *rc_task)
{
    H264eSps *sps = ctx->sps;
    H264eSlice *slice = ctx->slice;
    MppEncCfgSet *cfg = ctx->cfg;
...
static void setup_vepu511a_io_buf(HalVepu511aRegSet *regs, MppDevRegOffCfgs *offsets,
                                  HalEncTask *task)
{
...
static MPP_RET vepu511a_h264_set_one_roi(void *buf, MppEncROIRegion *region, RK_S32 w, RK_S32 h)
{
    Vepu511aRoiH264BsCfg *ptr = (Vepu511aRoiH264BsCfg *)buf;
    RK_S32 mb_w = MPP_ALIGN(w, 16) / 16;
    RK_S32 mb_h = MPP_ALIGN(h, 16) / 16;
    RK_S32 stride_h = MPP_ALIGN(mb_w, 4);
    Vepu511aRoiH264BsCfg cfg;
    MPP_RET ret = MPP_NOK;
...
static MPP_RET setup_vepu511a_intra_refresh(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx, RK_U32 refresh_idx)
{
    MPP_RET ret = MPP_OK;
    ...
    Vepu511aRoiH264BsCfg base_cfg;
    Vepu511aRoiH264BsCfg *base_cfg_ptr = (Vepu511aRoiH264BsCfg *)base_cfg_buf;
    ...
    if (NULL == region) {
        mpp_loge_f("Failed to calloc for MppEncROIRegion !\n");
        ret = MPP_ERR_MALLOC;
    }
    ...
    vepu511a_h264_set_one_roi(base_cfg_buf, region, w, h);
    mpp_free(region);
RET:
...
static void setup_vepu511a_recn_refr(HalH264eVepu511aCtx *ctx, HalVepu511aRegSet *regs)
{
...
static void setup_vepu511a_split(HalVepu511aRegSet *regs, MppEncCfgSet *enc_cfg)
{
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
    ...
    case MPP_ENC_SPLIT_BY_CTU : {
        ...
        if ((cfg->split_out & MPP_ENC_SPLIT_OUT_LOWDELAY) ||
            (regs->reg_frm.common.enc_pic.slen_fifo && (slice_num > VEPU511A_SLICE_FIFO_LEN)))
...
static void setup_vepu511a_me(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
    H264eVepu511aParam *reg_param = &regs->reg_param;
...
static void vepu511a_set_speed(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
    H264eVepu511aParam *reg_param = &regs->reg_param;
...
static void
setup_vepu511a_l2(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    MppEncSceneMode sm = ctx->cfg->tune.scene_mode;
...
static void setup_vepu511a_ext_line_buf(HalVepu511aRegSet *regs, HalH264eVepu511aCtx *ctx)
{
    H264eVepu511aFrame *reg_frm = &regs->reg_frm;
...
static void setup_vepu511a_aq(HalH264eVepu511aCtx *ctx)
{
    MppEncCfgSet *cfg = ctx->cfg;
    MppEncHwCfg *hw = &cfg->hw;
    Vepu511aRcRoi *s = &ctx->regs_set->reg_rc_roi;
...
static void setup_vepu511a_anti_ringing(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    H264eVepu511aSqi *s = &regs->reg_sqi;
    MppEncSceneMode sm = ctx->cfg->tune.scene_mode;
...
static void setup_vepu511a_anti_flicker(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
...
static void setup_vepu511a_anti_smear(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    H264eVepu511aSqi *reg = &regs->reg_sqi;
    H264eSlice *slice = ctx->slice;
...
static void setup_vepu511a_scaling_list(HalH264eVepu511aCtx *ctx)
{
    HalVepu511aRegSet *regs = ctx->regs_set;
    H264eVepu511aSclCfg *s = &regs->reg_scl;
    ...
    if (ctx->pps->pic_scaling_matrix_present == 1) {
        for (idx = 0; idx < 64; idx++) {
            p[idx] = vepu511a_h264_cqm_jvt8i[63 - idx]; /* intra8x8 */
            p[idx + 64] = vepu511a_h264_cqm_jvt8p[63 - idx]; /* inter8x8 */
        }
...
static MPP_RET hal_h264e_vepu511a_gen_regs(void *hal, HalEncTask *task)
{
    ...
    setup_vepu511a_normal(regs);
    ret = setup_vepu511a_prep(regs, &ctx->cfg->prep, task);
    if (ret)
        return ret;

    setup_vepu511a_codec(regs, ctx);
    setup_vepu511a_rdo_pred(ctx);
    setup_vepu511a_aq(ctx);
    setup_vepu511a_anti_stripe(ctx);
    setup_vepu511a_anti_ringing(ctx);
    ...
    setup_vepu511a_recn_refr(ctx, regs);
    setup_vepu511a_split(regs, cfg);
    setup_vepu511a_me(ctx);
    vepu511a_set_speed(ctx);

    if (frm->is_i_refresh)
        setup_vepu511a_intra_refresh(regs, ctx, frm->seq_idx % cfg->rc.gop);

    setup_vepu511a_l2(ctx);
    setup_vepu511a_ext_line_buf(regs, ctx);
    ...
    if (frm->save_pass1)
        vepu511a_h264e_save_pass1_patch(regs, ctx);

    if (frm->use_pass1)
        vepu511a_h264e_use_pass1_patch(regs, ctx);
...
static MPP_RET hal_h264e_vepu511a_start(void *hal, HalEncTask *task)
{
    MPP_RET ret = MPP_OK;
    HalH264eVepu511aCtx *ctx = (HalH264eVepu511aCtx *)hal;
    HalVepu511aRegSet *regs = ctx->regs_set;
    ...
    do {
        ...
        wr_cfg.offset = VEPU511A_CTL_OFFSET;
#if DUMP_REG
        {
            RK_U32 i;
        ...
        wr_cfg.offset = VEPU511A_FRAME_OFFSET;
        ...
        wr_cfg.offset = VEPU511A_RC_ROI_OFFSET;
        ...
        wr_cfg.offset = VEPU511A_PARAM_OFFSET;
        ...
        wr_cfg.offset = VEPU511A_SQI_OFFSET;
        ...
        wr_cfg.offset = VEPU511A_SCL_OFFSET ;
        ...
        rd_cfg.offset = VEPU511A_REG_BASE_HW_STATUS;
        ...
        rd_cfg.offset = VEPU511A_STATUS_OFFSET;
...
static MPP_RET hal_h264e_vepu511a_status_check(HalVepu511aRegSet *regs)
{
    MPP_RET ret = MPP_OK;
...
static MPP_RET hal_h264e_vepu511a_wait(void *hal, HalEncTask *task)
{
    MPP_RET ret = MPP_OK;
    HalH264eVepu511aCtx *ctx = (HalH264eVepu511aCtx *)hal;
    HalVepu511aRegSet *regs = ctx->regs_set;
    ...
    if (split_out) {
        ...
        if (ret) {
            ...
            ret = hal_h264e_vepu511a_status_check(regs);
            if (!ret)
...
static MPP_RET hal_h264e_vepu511a_ret_task(void * hal, HalEncTask * task)
{
    ...
    vepu511a_h264e_update_tune_stat(ctx, task);
...
const MppEncHalApi hal_h264e_vepu511a = {
    .name       = "hal_h264e_vepu511a",
    .coding     = MPP_VIDEO_CodingAVC,
    .ctx_size   = sizeof(HalH264eVepu511aCtx),
    .flag       = 0,
    .init       = hal_h264e_vepu511a_init,
    .deinit     = hal_h264e_vepu511a_deinit,
    .prepare    = hal_h264e_vepu511a_prepare,
    .get_task   = hal_h264e_vepu511a_get_task,
    .gen_regs   = hal_h264e_vepu511a_gen_regs,
    .start      = hal_h264e_vepu511a_start,
    .wait       = hal_h264e_vepu511a_wait,
    .part_start = NULL,
    .part_wait  = NULL,
    .ret_task   = hal_h264e_vepu511a_ret_task,
...
MPP_ENC_HAL_API_REGISTER(hal_h264e_vepu511a)
