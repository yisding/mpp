...
typedef struct Vepu511H264Fbk_t {
...
typedef struct HalH264eVepu511Ctx_t {
    ...
    Vepu511H264Fbk          feedback;
    Vepu511H264Fbk          last_frame_fb;
...
static void setup_ext_line_bufs(HalH264eVepu511Ctx *ctx)
{
    if (ctx->ext_line_buf)
        return;
...
static MPP_RET hal_h264e_vepu511_deinit(void *hal)
{
    ...
    if (p->tune) {
        // vepu511_h264e_tune_deinit(p->tune);
        p->tune = NULL;
    }
...
static MPP_RET hal_h264e_vepu511_init(void *hal, MppEncHalCfg *cfg)
{
    ...
    p->regs_set = mpp_malloc(HalVepu511RegSet, 1);
    if (NULL == p->regs_set) {
        ret = MPP_ERR_MALLOC;
    ...
    if (NULL == p->reorder) {
        ret = MPP_ERR_MALLOC;
        mpp_err_f("init reorder data failed\n");
        goto DONE;
    }
    ...
    if (NULL == p->marking) {
        ret = MPP_ERR_MALLOC;
        mpp_err_f("init marking data failed\n");
        goto DONE;
    }
    ...
    // p->tune = vepu511_h264e_tune_init(p);
...
static MPP_RET setup_vepu511_prep(HalVepu511RegSet *regs, MppEncPrepCfg *prep, HalEncTask *task)
{
    VepuFmtCfg cfg;
...
static void setup_vepu511_codec(HalVepu511RegSet *regs, HalH264eVepu511Ctx *ctx)
{
    ...
    regs->reg_frm.synt_sps.poc_type       = sps->pic_order_cnt_type;
...
static void setup_vepu511_rdo_pred(HalH264eVepu511Ctx *ctx)
{
...
static void setup_vepu511_rc_base(HalVepu511RegSet *regs, HalH264eVepu511Ctx *ctx, EncRcTask *rc_task)
{
    H264eSps *sps = ctx->sps;
    H264eSlice *slice = ctx->slice;
    MppEncCfgSet *cfg = ctx->cfg;
...
static void setup_vepu511_io_buf(HalVepu511RegSet *regs, MppDevRegOffCfgs *offsets,
                                 HalEncTask *task)
{
...
static MPP_RET vepu511_h264_set_one_roi(void *buf, MppEncROIRegion *region, RK_S32 w, RK_S32 h)
{
    Vepu511RoiH264BsCfg *ptr = (Vepu511RoiH264BsCfg *)buf;
...
static MPP_RET setup_vepu511_intra_refresh(HalVepu511RegSet *regs, HalH264eVepu511Ctx *ctx, RK_U32 refresh_idx)
{
    MPP_RET ret = MPP_OK;
...
static void setup_vepu511_split(HalVepu511RegSet *regs, MppEncCfgSet *enc_cfg)
{
    ...
    default : {
        mpp_logi_f("invalide slice split mode %d\n", cfg->split_mode);
    } break;
    }
...
static void
setup_vepu511_l2(HalH264eVepu511Ctx *ctx)
{
    HalVepu511RegSet *regs = ctx->regs_set;
    MppEncSceneMode sm = ctx->cfg->tune.scene_mode;
...
static void setup_vepu511_aq(HalH264eVepu511Ctx *ctx)
{
    ...
    s->aq_tthd0.aq_tthd0  = (RK_U8)aq_thd[0] & 0xff;
    s->aq_tthd0.aq_tthd1  = (RK_U8)aq_thd[1] & 0xff;
    s->aq_tthd0.aq_tthd2  = (RK_U8)aq_thd[2] & 0xff;
    s->aq_tthd0.aq_tthd3  = (RK_U8)aq_thd[3] & 0xff;
    s->aq_tthd1.aq_tthd4  = (RK_U8)aq_thd[4] & 0xff;
    s->aq_tthd1.aq_tthd5  = (RK_U8)aq_thd[5] & 0xff;
    s->aq_tthd1.aq_tthd6  = (RK_U8)aq_thd[6] & 0xff;
    s->aq_tthd1.aq_tthd7  = (RK_U8)aq_thd[7] & 0xff;
    s->aq_tthd2.aq_tthd8  = (RK_U8)aq_thd[8] & 0xff;
    s->aq_tthd2.aq_tthd9  = (RK_U8)aq_thd[9] & 0xff;
    s->aq_tthd2.aq_tthd10 = (RK_U8)aq_thd[10] & 0xff;
    s->aq_tthd2.aq_tthd11 = (RK_U8)aq_thd[11] & 0xff;
    s->aq_tthd3.aq_tthd12 = (RK_U8)aq_thd[12] & 0xff;
    s->aq_tthd3.aq_tthd13 = (RK_U8)aq_thd[13] & 0xff;
    s->aq_tthd3.aq_tthd14 = (RK_U8)aq_thd[14] & 0xff;
    s->aq_tthd3.aq_tthd15 = (RK_U8)aq_thd[15] & 0xff;
...
static void setup_vepu511_anti_smear(HalH264eVepu511Ctx *ctx)
{
    ...
    flg3 = (flg1 != 0) ? 3 : (smear_cnt[4] > ((102 * mb_cnt) >> 10)) ? 2 :
    ...
    reg->rdo_smear_st_thd.madp_cnt_th0 = (flg2 != 0) ? 0 : 1;
    reg->rdo_smear_st_thd.madp_cnt_th1 = (flg2 != 0) ? 0 : 5;
    reg->rdo_smear_st_thd.madp_cnt_th2 = (flg2 != 0) ? 0 : 1;
    reg->rdo_smear_st_thd.madp_cnt_th3 = (flg2 != 0) ? 0 : 3;
...
static MPP_RET hal_h264e_vepu511_gen_regs(void *hal, HalEncTask *task)
{
    ...
    setup_vepu511_codec(regs, ctx);
    setup_vepu511_rdo_pred(ctx);
    ...
    setup_vepu511_rc_base(regs, ctx, rc_task);
    setup_vepu511_io_buf(regs, ctx->offsets, task);
    ...
    if (ctx->roi_data)
        vepu511_set_roi(&ctx->regs_set->reg_rc_roi.roi_cfg, ctx->roi_data,
...
static MPP_RET hal_h264e_vepu511_start(void *hal, HalEncTask *task)
{
    ...
    do {
        ...
        wr_cfg.reg = &regs->reg_scl_jpgtbl;
        wr_cfg.size = sizeof(regs->reg_scl_jpgtbl);
        wr_cfg.offset = VEPU511_SCL_JPGTBL_OFFSET ;
...
MPP_ENC_HAL_API_REGISTER(hal_h264e_vepu511)
