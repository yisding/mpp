...
#define MODULE_TAG  "hal_h265e_v511a"
...
typedef struct Vepu511aH265eFrmCfg_t {
    ...
    /* regs cfg */
    H265eV511ARegSet     *regs_set;
    H265eV511AStatusElem *regs_ret;
...
static void clear_ext_line_bufs(H265eV511AHalContext *ctx)
{
    if (ctx->ext_line_buf) {
...
static MPP_RET vepu511a_h265_setup_hal_bufs(H265eV511AHalContext *ctx)
{
...
MPP_RET hal_h265e_vepu511a_deinit(void *hal)
{
    ...
    if (ctx->frm) {
        if (ctx->frm->roir_buf) {
            mpp_buffer_put(ctx->frm->roir_buf);
            ctx->frm->roir_buf = NULL;
            ctx->frm->roir_buf_size = 0;
        }

        MPP_FREE(ctx->frm->roi_base_cfg_sw_buf);

        if (ctx->frm->reg_cfg) {
            mpp_dev_multi_offset_deinit(ctx->frm->reg_cfg);
            ctx->frm->reg_cfg = NULL;
        }

        MPP_FREE(ctx->frm->regs_set);
        MPP_FREE(ctx->frm->regs_ret);
        MPP_FREE(ctx->frm);
    }
...
MPP_RET hal_h265e_vepu511a_init(void *hal, MppEncHalCfg *cfg)
{
    MPP_RET ret = MPP_OK;
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    ...
    /* single core uses single frame config */
    ctx->frm = mpp_calloc(Vepu511aH265eFrmCfg, 1);
    ctx->frm->regs_set = mpp_calloc(H265eV511ARegSet, 1);
    ctx->frm->regs_ret = mpp_calloc(H265eV511AStatusElem, 1);
    ...
    if (ret) {
        mpp_loge_f("mpp_dev_init failed. ret: %d\n", ret);
        return ret;
    }
...
DONE:
    if (ret)
        hal_h265e_vepu511a_deinit(hal);

    hal_h265e_leave();
    return ret;
}

static MPP_RET hal_h265e_vepu511a_prepare(void *hal)
{
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    MppEncPrepCfg *prep = &ctx->cfg->prep;
    ...
    if (prep->change_res) {
        ...
        vepu511a_h265_setup_hal_bufs(ctx);
...
static MPP_RET
vepu511a_h265_set_patch_info(H265eSyntax_new *syn, VepuFmt input_fmt, MppDevRegOffCfgs *offsets, HalEncTask *task)
{
...
static MPP_RET vepu511a_h265e_save_pass1_patch(H265eV511ARegSet *regs, H265eV511AHalContext *ctx,
                                               RK_S32 tiles_enabled_flag)
{
    ...
    reg_frm->common.me_ref_comb.rfpw_mode = 0;
...
static MPP_RET vepu511a_h265e_use_pass1_patch(H265eV511ARegSet *regs, H265eV511AHalContext *ctx)
{
    Vepu511aControlCfg *reg_ctl = &regs->reg_ctl;
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    ...
    reg_frm->common.src_proc.rkfbcd_en  = 0;
    ...
    if (ret)
        mpp_loge_f("set input cb addr offset failed %d\n", ret);

    return MPP_OK;
}

static void setup_vepu511a_ext_line_buf(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    RK_S32 fd;
...
static void vepu511a_h265_set_scaling_list(H265eV511ARegSet *regs)
{
    H265eVepu511aSclCfg *s = &regs->reg_scl;
...
static void vepu511a_h265_set_prep(void *hal, HalEncTask *task, H265eV511ARegSet *regs)
{
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    Vepu511aRcRoi *reg_klut = &regs->reg_rc_roi;
...
static void vepu511a_h265_set_split(H265eV511ARegSet *regs, MppEncCfgSet *enc_cfg)
{
...
static void vepu511a_h265_set_me_regs(H265eV511AHalContext *ctx, H265eSyntax_new *syn, H265eV511ARegSet *regs)
{
...
static void vepu511a_set_speed(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    H265eVepu511aParam *s = &regs->reg_param;
...
static void vepu511a_h265_set_hw_address(H265eV511AHalContext *ctx, H265eVepu511aFrame *regs,
                                         HalEncTask *task)
{
...
static void vepu511a_h265_set_pp_regs(H265eV511ARegSet *regs, VepuFmtCfg *fmt,
                                      MppEncPrepCfg *prep_cfg, HalEncTask *task)
{
    ...
    if (reg_frm->common.src_fmt.src_cfmt < VEPU5xx_FMT_ARGB1555) {
        const VepuRgb2YuvCfg *cfg_coeffs = get_rgb2yuv_cfg(prep_cfg->range, prep_cfg->color);
...
static void vepu511a_h265_set_rc_regs(H265eV511AHalContext *ctx, H265eV511ARegSet *regs,
                                      HalEncTask *task)
{
    H265eSyntax_new *syn = (H265eSyntax_new *)task->syntax.data;
    EncRcTaskInfo *rc_cfg = &task->rc_task->info;
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    Vepu511aRcRoi *reg_rc = &regs->reg_rc_roi;
...
static void vepu511a_h265_set_quant_regs(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
...
static void vepu511a_h265_set_atr_regs(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    ...
    if (str == 3) {
        ...
        s->line_opt_cfg.line_thre_min_cst_best_grdn_blk_dep1 = 255;
        s->line_opt_cfg.line_thre_min_cst_best_grdn_blk_dep2 = 255;
        s->line_opt_cfg.line_thre_ratio_best_grdn_blk_dep0   = 7;
        s->line_opt_cfg.line_thre_ratio_best_grdn_blk_dep1   = 7;
...
static void vepu511a_h265_set_smear_regs(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    H265eVepu511aSqi *s = &regs->reg_sqi;
...
static void vepu511a_h265_set_ref_regs(H265eSyntax_new *syn, H265eVepu511aFrame *regs)
{
...
static void vepu511a_h265_set_atf_regs(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    H265eVepu511aSqi *reg = &regs->reg_sqi;
    ...
    static RK_U8 b32_skip_wgt0[3] =  { 24, 14, 12 };
    static RK_U8 b32_skip_wgt1[3] =  { 20, 12, 10 };
    static RK_U8 b32_skip_wgt2[3] =  { 20, 12, 10 };
    static RK_U8 b16_skip_wgt0[3] =  { 24, 14, 12 };
    static RK_U8 b16_skip_wgt1[3] =  { 20, 10, 8 };
    static RK_U8 b16_skip_wgt2[3] =  { 20, 10, 8 };
    static RK_U8 b32_inter_wgt0[3] = { 24, 26, 28 };
    static RK_U8 b32_inter_wgt1[3] = { 20, 22, 26 };
    static RK_U8 b32_inter_wgt2[3] = { 20, 22, 26 };
    static RK_U8 b16_inter_wgt0[3] = { 32, 40, 44 };
    static RK_U8 b16_inter_wgt1[3] = { 28, 40, 44 };
    static RK_U8 b16_inter_wgt2[3] = { 28, 40, 44 };
    static RK_U8 b32_intra_wgt0[3] = { 32, 36, 42 };
    static RK_U8 b32_intra_wgt1[3] = { 32, 36, 42 };
    static RK_U8 b32_intra_wgt2[3] = { 32, 36, 42 };
    static RK_U8 b16_intra_wgt0[3] = { 40, 44, 48 };
    static RK_U8 b16_intra_wgt1[3] = { 40, 44, 48 };
    static RK_U8 b16_intra_wgt2[3] = { 40, 44, 48 };
    ...
    /* 0 - disable; 1 - weak; 2 - medium; 3 - strong */
    str_idx = str - 1;
    reg->dark_brgt_opt_cfg.thre_max_luma_dark = 20;
    reg->dark_brgt_opt_cfg.thre_min_luma_bright = 235;
    reg->dark_brgt_opt_cfg.thre_ratio_dark_bright = 48;

    p_rdo_b32_skip = &reg->rdo_b32_skip;
    p_rdo_b32_skip->atf_thd0.madp_thd0 = 1;
    p_rdo_b32_skip->atf_thd0.madp_thd1 = 20;
    p_rdo_b32_skip->atf_thd1.madp_thd2 = 40;
    p_rdo_b32_skip->atf_thd1.madp_thd3 = 60;
    p_rdo_b32_skip->atf_wgt0.wgt0 = b32_skip_wgt0[str_idx];
    p_rdo_b32_skip->atf_wgt0.wgt1 = b32_skip_wgt1[str_idx];
    p_rdo_b32_skip->atf_wgt0.wgt2 = b32_skip_wgt2[str_idx];
    p_rdo_b32_skip->atf_wgt0.wgt3 = 0;

    p_rdo_b32_noskip = &reg->rdo_b32_inter;
    p_rdo_b32_noskip->atf_thd0.madp_thd0 = 8;
    p_rdo_b32_noskip->atf_thd0.madp_thd1 = 16;
    p_rdo_b32_noskip->atf_thd1.madp_thd2 = 32;
    p_rdo_b32_noskip->atf_thd1.flckr_zero_mv_j_coef = 16;
    p_rdo_b32_noskip->atf_wgt.wgt0 = b32_inter_wgt0[str_idx];
    p_rdo_b32_noskip->atf_wgt.wgt1 = b32_inter_wgt1[str_idx];
    p_rdo_b32_noskip->atf_wgt.wgt2 = b32_inter_wgt2[str_idx];

    p_rdo_noskip = &reg->rdo_b32_intra;
    p_rdo_noskip->ratf_thd0.madp_thd0 = 400;
    p_rdo_noskip->ratf_thd0.madp_thd1 = 400;
    p_rdo_noskip->ratf_thd1.madp_thd2 = 400;
    p_rdo_noskip->atf_wgt.wgt0 = b32_intra_wgt0[str_idx];
    p_rdo_noskip->atf_wgt.wgt1 = b32_intra_wgt1[str_idx];
    p_rdo_noskip->atf_wgt.wgt2 = b32_intra_wgt2[str_idx];

    p_rdo_skip = &reg->rdo_b16_skip;
    p_rdo_skip->atf_thd0.madp_thd0 = 1;
    p_rdo_skip->atf_thd0.madp_thd1 = 20;
    p_rdo_skip->atf_thd1.madp_thd2 = 40;
    p_rdo_skip->atf_thd1.madp_thd3 = 60;
    p_rdo_skip->atf_wgt0.wgt0 = b16_skip_wgt0[str_idx];
    p_rdo_skip->atf_wgt0.wgt1 = b16_skip_wgt1[str_idx];
    p_rdo_skip->atf_wgt0.wgt2 = b16_skip_wgt2[str_idx];
    p_rdo_skip->atf_wgt0.wgt3 = 2;

    p_rdo_noskip = &reg->rdo_b16_inter;
    p_rdo_noskip->ratf_thd0.madp_thd0 = 8;
    p_rdo_noskip->ratf_thd0.madp_thd1 = 16;
    p_rdo_noskip->ratf_thd1.madp_thd2 = 32;
    p_rdo_noskip->atf_wgt.wgt0 = b16_inter_wgt0[str_idx];
    p_rdo_noskip->atf_wgt.wgt1 = b16_inter_wgt1[str_idx];
    p_rdo_noskip->atf_wgt.wgt2 = b16_inter_wgt2[str_idx];

    p_rdo_noskip = &reg->rdo_b16_intra;
    p_rdo_noskip->ratf_thd0.madp_thd0 = 150;
    p_rdo_noskip->ratf_thd0.madp_thd1 = 150;
    p_rdo_noskip->ratf_thd1.madp_thd2 = 150;
    p_rdo_noskip->atf_wgt.wgt0 = b16_intra_wgt0[str_idx];
    p_rdo_noskip->atf_wgt.wgt1 = b16_intra_wgt1[str_idx];
    p_rdo_noskip->atf_wgt.wgt2 = b16_intra_wgt2[str_idx];
    p_rdo_noskip->atf_wgt.wgt3 = 25;
}

static void vepu511a_h265_set_aq(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    MppEncHwCfg *hw = &ctx->cfg->hw;
    Vepu511aRcRoi *rc_regs =  &regs->reg_rc_roi;
...
static void vepu511a_h265_global_cfg_set(H265eV511AHalContext *ctx, H265eV511ARegSet *regs)
{
    H265eVepu511aFrame *reg_frm = &regs->reg_frm;
    H265eVepu511aParam *reg_param = &regs->reg_param;
...
MPP_RET hal_h265e_vepu511a_gen_regs(void *hal, HalEncTask *task)
{
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    Vepu511aH265eFrmCfg *frm_cfg = ctx->frm;
    H265eV511ARegSet *regs = frm_cfg->regs_set;
    MPP_RET ret = MPP_OK;
    ...
    memset(regs, 0, sizeof(H265eV511ARegSet));

    vepu511a_h265_set_normal(ctx, regs);
    vepu511a_h265_set_prep(ctx, task, regs);
    vepu511a_h265_set_me_regs(ctx, syn , regs);
    vepu511a_h265_set_split(regs, ctx->cfg);
    vepu511a_set_speed(ctx, regs);
    vepu511a_h265_set_hw_address(ctx, reg_frm, task);
    vepu511a_h265_set_pp_regs(regs, fmt, &ctx->cfg->prep, task);
    ...
    ret = vepu511a_h265_set_patch_info(syn, (VepuFmt)fmt->format, ctx->reg_cfg, enc_task);
    if (ret)
        return ret;

    setup_vepu511a_ext_line_buf(ctx, regs);
    vepu511a_h265_set_atf_regs(ctx, regs);
    vepu511a_h265_set_anti_stripe_regs(ctx, regs);
    vepu511a_h265_set_atr_regs(ctx, regs);
    vepu511a_h265_set_smear_regs(ctx, regs);
    vepu511a_h265_set_scaling_list(regs);
    vepu511a_h265_set_aq(ctx, regs);
    vepu511a_h265_set_subj_opt_regs(regs);

    if (ctx->roi_data)
        vepu511a_set_roi(&regs->reg_rc_roi.roi_cfg, ctx->roi_data,
    ...
    if (frm->save_pass1)
        vepu511a_h265e_save_pass1_patch(regs, ctx, syn->pp.tiles_enabled_flag);

    if (frm->use_pass1)
        vepu511a_h265e_use_pass1_patch(regs, ctx);
...
MPP_RET hal_h265e_vepu511a_start(void *hal, HalEncTask *enc_task)
{
    MPP_RET ret = MPP_OK;
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    Vepu511aH265eFrmCfg *frm = ctx->frm;
    RK_U32 *regs = (RK_U32*)frm->regs_set;
    H265eV511ARegSet *hw_regs = frm->regs_set;
    H265eV511AStatusElem *reg_out = (H265eV511AStatusElem *)frm->regs_ret;
    MppDevRegWrCfg cfg;
    MppDevRegRdCfg cfg1;
    RK_U32 i = 0;
    ...
    cfg.offset = VEPU511A_CTL_OFFSET;

    ret = mpp_dev_ioctl(ctx->dev, MPP_DEV_REG_WR, &cfg);
    if (ret) {
        mpp_loge_f("set register write failed %d\n", ret);
        return ret;
    }

    if (hal_h265e_debug & HAL_H265E_DBG_CTL_REGS) {
        regs = (RK_U32*)&hw_regs->reg_ctl;
        for (i = 0; i < sizeof(Vepu511aControlCfg) / 4; i++) {
            hal_h265e_dbg_ctl("ctl reg[%04x]: 0%08x\n", i * 4, regs[i]);
        }
    }

    cfg.reg = &hw_regs->reg_frm;
    cfg.size = sizeof(H265eVepu511aFrame);
    cfg.offset = VEPU511A_FRAME_OFFSET;
    ...
    if (hal_h265e_debug & HAL_H265E_DBG_REGS) {
        ...
        regs += 32;
        for (i = 0; i < (sizeof(H265eVepu511aFrame) - 128) / 4; i++) {
            hal_h265e_dbg_regs("set reg[%04x]: 0%08x\n", i * 4, regs[i]);
        }
    }
    cfg.reg = &hw_regs->reg_rc_roi;
    cfg.size = sizeof(Vepu511aRcRoi);
    cfg.offset = VEPU511A_RC_ROI_OFFSET;
    ...
    if (hal_h265e_debug & HAL_H265E_DBG_RCKUT_REGS) {
        ...
        for (i = 0; i < sizeof(Vepu511aRcRoi) / 4; i++) {
            hal_h265e_dbg_rckut("set rc roi reg[%04x]: 0%08x\n", i * 4, regs[i]);
        }
    }

    cfg.reg =  &hw_regs->reg_param;
    cfg.size = sizeof(H265eVepu511aParam);
    cfg.offset = VEPU511A_PARAM_OFFSET;
    ...
    if (hal_h265e_debug & HAL_H265E_DBG_WGT_REGS) {
        ...
        for (i = 0; i < sizeof(H265eVepu511aParam) / 4; i++) {
            hal_h265e_dbg_wgt("set param reg[%04x]: 0%08x\n", i * 4, regs[i]);
        }
    }

    cfg.reg = &hw_regs->reg_sqi;
    cfg.size = sizeof(H265eVepu511aSqi);
    cfg.offset = VEPU511A_SQI_OFFSET;
    ...
    if (hal_h265e_debug & HAL_H265E_DBG_WGT_REGS) {
        ...
        for (i = 0; i < sizeof(H265eVepu511aSqi) / 4; i++) {
    ...
    cfg.offset = VEPU511A_SCL_OFFSET ;
    ...
    if (hal_h265e_debug & HAL_H265E_DBG_WGT_REGS) {
        ...
        for (i = 0; i < sizeof(H265eVepu511aSclCfg) / 4; i++) {
    ...
    cfg1.offset = VEPU511A_REG_BASE_HW_STATUS;

    ret = mpp_dev_ioctl(ctx->dev, MPP_DEV_REG_RD, &cfg1);
    if (ret) {
        mpp_loge_f("set register read failed %d\n", ret);
        return ret;
    }

    cfg1.reg = &reg_out->st;
    cfg1.size = sizeof(H265eV511AStatusElem) - 4;
    cfg1.offset = VEPU511A_STATUS_OFFSET;
    ...
    if (ret) {
        mpp_loge_f("send cmd failed %d\n", ret);
    }
    hal_h265e_leave();
    return ret;
}

static MPP_RET vepu511a_h265_set_feedback(H265eV511AHalContext *ctx, HalEncTask *enc_task)
{
    EncRcTaskInfo *hal_rc_ret = (EncRcTaskInfo *)&enc_task->rc_task->info;
    Vepu511aH265Fbk  *fb = &ctx->frm->feedback;
    MppEncCfgSet    *cfg = ctx->cfg;
    RK_S32 mb8_num = MPP_ALIGN(cfg->prep.width, 8) * MPP_ALIGN(cfg->prep.height, 8) / 64;
    RK_S32 mb4_num = (mb8_num << 2);
    H265eV511AStatusElem *elem = (H265eV511AStatusElem *)ctx->frm->regs_ret;
    ...
    ctx->feedback.acc_zero_mv = elem->st.st_mv_zero_sum.acc_zero_mv;
...
static MPP_RET hal_h265e_vepu511a_status_check(H265eV511ARegSet *regs)
{
    MPP_RET ret = MPP_OK;
...
static void vepu511a_h265e_update_tune_stat(H265eV511AHalContext *ctx, HalEncTask *task)
{
    Vepu511aH265Fbk *fb = &ctx->frm->feedback;
    H265eV511ARegSet *regs = ctx->frm->regs_set;
    Vepu511aRcRoi *s = &regs->reg_rc_roi;
    MppEncCfgSet *cfg = ctx->cfg;
...
MPP_RET hal_h265e_vepu511a_wait(void *hal, HalEncTask *task)
{
    ...
    if (split_out) {
        ...
        do {
             ...
             */
            if (ret) {
                mpp_loge_f("slice poll failed %d after %d slice(s)\n",
            ...
            if (poll_cfg->count_ret > 0) {
                ...
                mpp_loge_f("slice poll returned no slice %d times\n", empty_cnt);
                ret = MPP_ERR_VPUHW;
                break;
            }
        } while (!slice_last);
        ...
        if (ret) {
            ...
            ret = hal_h265e_vepu511a_status_check(regs);
            if (!ret)
...
MPP_RET hal_h265e_vepu511a_get_task(void *hal, HalEncTask *task)
{
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    ...
    ctx->frm->frame_count = ++ctx->frame_count;
    task->part_first = 1;
    task->part_last = 0;

    ctx->frm->hal_curr_idx = ctx->syn->sp.recon_pic.slot_idx;
    ctx->frm->hal_refr_idx = ctx->syn->sp.ref_pic.slot_idx;

    h265e_dpb_hal_start(ctx->dpb, ctx->frm->hal_curr_idx);
    h265e_dpb_hal_start(ctx->dpb, ctx->frm->hal_refr_idx);

    memset(&ctx->frm->feedback, 0, sizeof(Vepu511aH265Fbk));

    hal_h265e_leave();
    return MPP_OK;
}

MPP_RET hal_h265e_vepu511a_ret_task(void *hal, HalEncTask *task)
{
    H265eV511AHalContext *ctx = (H265eV511AHalContext *)hal;
    HalEncTask *enc_task = task;
    Vepu511aH265Fbk *fb = &ctx->frm->feedback;
    ...
    vepu511a_h265_set_feedback(ctx, enc_task);
    ...
    h265e_dpb_hal_end(ctx->dpb, ctx->frm->hal_curr_idx);
    h265e_dpb_hal_end(ctx->dpb, ctx->frm->hal_refr_idx);

    vepu511a_h265e_update_tune_stat(ctx, enc_task);
...
const MppEncHalApi hal_h265e_vepu511a = {
    .name       = "hal_h265e_v511a",
    .coding     = MPP_VIDEO_CodingHEVC,
    .ctx_size   = sizeof(H265eV511AHalContext),
    .flag       = 0,
    .init       = hal_h265e_vepu511a_init,
    .deinit     = hal_h265e_vepu511a_deinit,
    .prepare    = hal_h265e_vepu511a_prepare,
    .get_task   = hal_h265e_vepu511a_get_task,
    .gen_regs   = hal_h265e_vepu511a_gen_regs,
    .start      = hal_h265e_vepu511a_start,
    .wait       = hal_h265e_vepu511a_wait,
    .part_start = NULL,
    .part_wait  = NULL,
    .ret_task   = hal_h265e_vepu511a_ret_task,
...
MPP_ENC_HAL_API_REGISTER(hal_h265e_vepu511a)
